...
class FastDFSStorage(Storage):
    ...
    def _open(self,name,mode='rb'):
        ...
        mode：文件打开方式
        """
        # 因为当前不是去打开某个文件，所以这个方法目前无用，但是又必须重写，所以用pass
        pass
    def _save(self,name,content):
        """
        在将来后台管理系统中，需要在这个方法中需要实现文件上传到FastDFS服务器
        :param name: 文件路径
        :param content: 文件二进制内容
        :return: None
        """
        # 因为当前不是去保存文件，所以这个方法目前无用，但是又必须重写，所以用pass
        pass
    def url(self,name):
        """
        返回文件的全路径
        ...
        """
        # return 'http://192.168.206.132:8888/'+ name
