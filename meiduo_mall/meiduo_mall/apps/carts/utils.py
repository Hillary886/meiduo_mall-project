...
def merge_cart_cookies_redis(request,user,response):
    ...
    # 将cart_str_bytes转成bytes类型的字符串
    cookie_cart_str_bytes = cart_str.encode()
    # 将cart_str_bytes转成bytes类型的字典
    cookie_cart_str_bytes = base64.b64decode(cookie_cart_str_bytes)
    # 将cart_dict_bytes转成真正的字典
    cookie_cart_dict = pickle.loads(cookie_cart_str_bytes)


    # 如果不存在，不需要合并
    # 如果存在，需要合并

    # 准备新的数据容器：保存新的sku_id:count,保存selected,unselected
    new_cart_dict={}
    new_selected_add=[]
    new_selected_rem=[]
    # 遍历出cookie中的购物车数据
